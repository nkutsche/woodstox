...
/**
 * Partial implementation of {@link XMLStreamReader2} consisting of all
 * functionality other than DTD-validation-specific parts, and Typed Access API
 * (Stax2 v3.0), which are implemented at sub-classes.
 *
...
public abstract class BasicStreamReader extends StreamScanner implements StreamReaderImpl, DTDInfo, LocationInfo {
	...
	final static int STATE_MULTIDOC_HACK = 3; // State "between" multiple
												// documents (in multi-doc mode)
	...
	/*
	 * minimum token length returnable achieved; only used for CHARACTERS event
	 * which allow fragments to be returned (and for CDATA in some limited
	 * cases)
	 */
	final static int TOKEN_PARTIAL_SINGLE = 2;

	/*
	 * a single physical event has been successfully tokenized; as with partial,
	 * only used with CDATA and CHARACTERS (meaningless for others, which should
	 * only use TOKEN_FULL_COALESCED, TOKEN_NOT_STARTED or TOKEN_STARTED.
	 */
	final static int TOKEN_FULL_SINGLE = 3;

	/*
	 * all adjacent (text) events have been tokenized and coalesced (for CDATA
	 * and CHARACTERS), or that the full event has been parsed (for others)
	 */
	...
	/**
	 * This mask covers all types for which basic {@link #getText} method can be
	 * called.
	 */
	final protected static int MASK_GET_TEXT = (1 << CHARACTERS) | (1 << CDATA) | (1 << SPACE) | (1 << COMMENT)
	 ...
	 */
	final protected static int MASK_GET_TEXT_XXX = (1 << CHARACTERS) | (1 << CDATA) | (1 << SPACE) | (1 << COMMENT);

	/**
	 * This mask is used with Stax2 getText() method (one that takes Writer as
	 * an argument): accepts even wider range of event types.
	 */
	final protected static int MASK_GET_TEXT_WITH_WRITER = (1 << CHARACTERS) | (1 << CDATA) | (1 << SPACE)
			| (1 << COMMENT) | (1 << DTD) | (1 << ENTITY_REFERENCE) | (1 << PROCESSING_INSTRUCTION);

	final protected static int MASK_GET_ELEMENT_TEXT = (1 << CHARACTERS) | (1 << CDATA) | (1 << SPACE)
			| (1 << ENTITY_REFERENCE);
	...
	/**
	 * Minimum number of characters parser can return as partial text segment,
	 * IF it's not required to coalesce adjacent text segments.
	 */
	...
	/**
	 * Object to notify about shared stuff, such as symbol tables, as well as to
	 * query for additional config settings if necessary.
	 */
	final protected ReaderCreator mOwner;

	/*
	 * /////////////////////////////////////////////////////////// // Additional
	 * XML document information, in addition // to what StreamScanner has
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * Status about "stand-aloneness" of document; set to 'yes'/'no'/'unknown'
	 * based on whether there was xml declaration, and if so, whether it had
	...
	/*
	 * /////////////////////////////////////////////////////////// // DOCTYPE
	 * information from document type declaration // (if any found)
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * Prefix of root element, as dictated by DOCTYPE declaration; null if no
	 * DOCTYPE declaration, or no root prefix
	 */
	protected String mRootPrefix;

	/**
	 * Local name of root element, as dictated by DOCTYPE declaration; null if
	...
	/**
	 * TextBuffer mostly used to collect non-element textual content (text,
	 * CDATA, comment content, pi data)
	 */
	 ...
	 * <p>
	 * Initially set to fully tokenized, since it's the virtual START_DOCUMENT
	 * event that we fully know by now (parsed by bootstrapper)
	 */
	protected int mTokenState = TOKEN_FULL_COALESCED;

	/**
	 * Threshold value that defines tokenization state that needs to be achieved
	 * to "finish" current <b>logical</b> text segment (which may consist of
	 * adjacent CDATA and text segments; or be a complete physical segment; or
	 * just even a fragment of such a segment)
	 */
	protected final int mStTextThreshold;

	/**
	 * Sized of currentTextLength for CDATA, CHARACTERS, WHITESPACE. When
	 * segmenting, this records to size of all the segments so we can track if
	 * the text length has exceeded limits.
	 */
	 ...
	 * {@link #getEventType}. Needs to be initialized to START_DOCUMENT, since
	 * that's the state it starts in.
	 */
	protected int mCurrToken = START_DOCUMENT;

	/**
	 * Additional information sometimes stored (when generating dummy events in
	 * multi-doc mode, for example) temporarily when {@link #mCurrToken} is
	 * already populated.
	 */
	protected int mSecondaryToken = START_DOCUMENT;

	/**
	 * Status of current (text) token's "whitespaceness", that is, whether it is
	 * or is not all white space.
	 */
	protected int mWsStatus;

	/**
	 * Flag that indicates that textual content (CDATA, CHARACTERS) is to be
	 * validated within current element's scope. Enabled if one of validators
	 * returns {@link XMLValidator#CONTENT_ALLOW_VALIDATABLE_TEXT}, and will
	 * prevent lazy parsing of text.
	 */
	...
	/**
	 * Due to the way Stax API does not allow throwing stream exceptions from
	 * many methods for which Woodstox would need to throw one (especially
	 * <code>getText</code> and its variations), we may need to delay throwing
	 * an exception until {@link #next} is called next time. If so, this
	 * variable holds the pending stream exception.
	 */
	protected XMLStreamException mPendingException = null;

	/*
	 * /////////////////////////////////////////////////////////// // DTD
	 * information (entities, content spec stub)
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * Entities parsed from internal/external DTD subsets. Although it will
	 * remain null for this class, extended classes make use of it, plus, to be
	 * able to share some of entity resolution code, instance is left here even
	 * though it semantically belongs to the sub-class.
	 */
	protected Map<String, EntityDecl> mGeneralEntities = null;

	/**
	 * Mode information needed at this level; mostly to check what kind of
	 * textual content (if any) is allowed in current element context. Constants
	 * come from {@link XMLValidator}, (like
	 * {@link XMLValidator#CONTENT_ALLOW_VALIDATABLE_TEXT}). Only used inside
	 * tree; ignored for prolog/epilog (which have straight-forward static
	 * rules).
	 */
	...
	/**
	 * @param elemStack
	 *            Input element stack to use; if null, will create instance
	 *            locally.
	 * @param forER
	 *            Override indicator; if true, this stream reader will be used
	 *            by an event reader, and should modify some of the base config
	 *            settings appropriately. If false, configuration settings are
	 *            to be used as is.
	 */
	protected BasicStreamReader(InputBootstrapper bs, BranchingReaderSource input, ReaderCreator owner,
			ReaderConfig cfg, InputElementStack elemStack, boolean forER) throws XMLStreamException {
		...
		/*
		 * 30-Sep-2005, TSa: Let's not do lazy parsing when access is via Event
		 * API. Reason is that there will be no performance benefit (event
		 * objects always access full info right after traversal), but the
		 * wrapping of stream exceptions within runtime exception wrappers would
		 * happen, which is inconvenient (loss of stack trace, not catching all
		 * exceptions as expected)
		 */
		mCfgLazyParsing = !forER && ((mConfigFlags & CFG_LAZY_PARSING) != 0);

		/*
		 * There are a few derived settings used during tokenization that need
		 * to be initialized now...
		 */
		if (mCfgCoalesceText) {
		...
		/*
		 * Ok; either we got declaration or not, but in either case we can now
		 * initialize prolog parsing settings, without having to really parse
		 * anything more.
		 */
		/*
		 * 07-Oct-2005, TSa: Except, if we are in fragment mode, in which case
		 * we are kind of "in tree" mode...
		 */
		mParseState = mConfig.inputParsingModeFragment() ? STATE_TREE : STATE_PROLOG;
	...
	/**
	 * As per Stax (1.0) specs, needs to return whatever xml declaration claimed
	 * encoding is, if any; or null if no xml declaration found.
	 * <p>
	...
	/**
	 * As per Stax (1.0) specs, needs to return whatever parser determined the
	 * encoding was, if it was able to figure it out. If not (there are cases
	 * where this can not be found; specifically when being passed a
	...
	public Object getProperty(String name) {
		/*
		 * 18-Nov-2008, TSa: As per [WSTX-50], should report the actual Base
		 * URL. It can be overridden by matching setProperty, but if not, is set
		 * to actual source of content being parsed.
		 */
		...
		/*
		 * 23-Apr-2008, TSa: Let's NOT throw IllegalArgumentException for
		 * unknown property; JavaDocs do not suggest it needs to be done
		 * (different from that of XMLInputFactory and XMLStreamWriter
		 * specification)
		 */
	...
	public String getElementText() throws XMLStreamException {
		 ...
		 * processed; that we are in-tree (not prolog or epilog). The only
		 * possible complication would be:
		 */
		if (mStEmptyElem) {
			/*
			 * And if so, we'll then get 'virtual' close tag; things are simple
			 * as location info was set when dealing with empty start element;
			 * and likewise, validation (if any) has been taken care of
			 */
			mStEmptyElem = false;
		...
		/*
		 * Ok: then a quick check; if it looks like we are directly followed by
		 * the end tag, we need not construct String quite yet.
		 */
		if ((mInputPtr + 1) < mInputEnd && mInputBuffer[mInputPtr] == '<' && mInputBuffer[mInputPtr + 1] == '/') {
	...
	/**
	 * Returns type of the last event returned; or START_DOCUMENT before any
	 * events has been explicitly returned.
	 */
	@Override
	public int getEventType() {
		/*
		 * Only complication -- multi-part coalesced text is to be reported as
		 * CHARACTERS always, never as CDATA (StAX specs).
		 */
		if (mCurrToken == CDATA) {
	...
	public String getLocalName() {
		...
		if (mCurrToken == ENTITY_REFERENCE) {
			/*
			 * 30-Sep-2005, TSa: Entity will be null in non-expanding mode if no
	...
	public NamespaceContext getNamespaceContext() {
		/*
		 * Unlike other getNamespaceXxx methods, this is available for all
		 * events. Note that the context is "live", ie. remains active (but not
		 * static) even through calls to next(). StAX compliant apps should not
		 * count on this behaviour, however.
		 */
	...
	public String getNamespacePrefix(int index) {
		...
		// Internally it's marked as null, externally need to see "" or null,
		// depending
	...
	public String getNamespaceURI(String prefix) {
		...
		/*
		 * Note: this will need to return null if no URI found for the prefix,
		 * so we can't mask it.
		 */
	...
	public boolean hasNext() {
		 ...
		 */
		return (mCurrToken != END_DOCUMENT) || (mParseState == STATE_MULTIDOC_HACK);
	}
	...
	public boolean isCharacters() {
		/*
		 * 21-Dec-2005, TSa: Changed for 3.0 to work the same way as stax ref
		 * impl.
		 */
		// return (mCurrToken == CHARACTERS || mCurrToken == CDATA || mCurrToken
		// == SPACE);
		/*
		 * 21-Apr-2009, TSa: As per [WSTX-201], should be consistent with what
		 * getEventType() returns (affects CDATA, SPACE, in coalescing mode or
		 * when explicitly asked to return CDATA as CHARACTERS)
		 */
	 ...
	 * read. That'd prevent double reads... but would it slow down that one
	 * reading so that net effect would be negative?
	 */
	@Override
	...
	public void require(int type, String nsUri, String localName) throws XMLStreamException {
		...
		/*
		 * There are some special cases; specifically, CDATA is sometimes
		 * reported as CHARACTERS. Let's be lenient by allowing both 'real' and
		 * reported types, for now.
		 */
		if (curr != type) {
			if (curr == CDATA) {
				if (mCfgCoalesceText || mCfgReportTextAsChars) {
		...
		if (type != curr) {
			throwParseError("Expected type " + tokenTypeDesc(type) + ", current type " + tokenTypeDesc(curr));
		}
		...
		if (nsUri != null) {
			if (curr != START_ELEMENT && curr != END_ELEMENT) {
				throwParseError("Expected non-null NS URI, but current token not a START_ELEMENT or END_ELEMENT (was "
						+ tokenTypeDesc(curr) + ")");
			}
			String uri = mElementStack.getNsURI();
			// No namespace?
			if (nsUri.length() == 0) {
				...
				if ((nsUri != uri) && !nsUri.equals(uri)) {
					throwParseError("Expected namespace '" + nsUri + "'; have '" + uri + "'.");
				}
			}
		}
		// Ok, fine, all's good
	...
	public final int next() throws XMLStreamException {
		 ...
		 * during parsing of the previous event. If so, now it needs to be
		 * thrown.
		 */
		...
		/*
		 * Note: can not yet accurately record the location, since the previous
		 * event might not yet be completely finished...
		 */
		if (mParseState == STATE_TREE) {
			...
			if (mTokenState < mStTextThreshold) { // incomplete?
				/*
				 * Can remain incomplete if lazy parsing is enabled, and this is
				 * not a validatable text segment; otherwise must finish
				 */
				if (!mCfgLazyParsing || (mValidateText && (type == CHARACTERS || type == CDATA))) {
					finishToken(false);
				}
			}

			/*
			 * Special cases -- sometimes (when coalescing text, or when
			 * specifically configured to do so), CDATA and SPACE are to be
			 * reported as CHARACTERS, although we still will internally keep
			 * track of the real type.
			 */
			if (type == CDATA) {
				if (mValidateText) {
				...
				/*
				 * } else if (type == SPACE) { //if (mValidateText) { throw new
				 * IllegalStateException("Internal error: trying to validate SPACE event"
				 * ); }
				 */
				...
				if (mValidateText) {
					/*
					 * We may be able to determine that there will be no more
					 * text coming for this element: but only seeing the end tag
					 * marker ("</") is certain (PIs and comments won't do, nor
					 * CDATA; start element possibly... but that indicates mixed
	...
	public int nextTag() throws XMLStreamException {
		while (true) {
			...
			case CDATA:
			case CHARACTERS:
				// inlined version of "isWhiteSpace()", so that exceptions can
				// be passed as-is
				// without suppression
				if (mTokenState < mStTextThreshold) {
					finishToken(false);
	 ...
	 * {@link org.codehaus.stax2.XMLInputFactory2#P_AUTO_CLOSE_INPUT} is set to
	 * true.
	 */
	@Override
	public void close() throws XMLStreamException {
		if (mParseState != STATE_CLOSED) {
			...
			/*
			 * Let's see if we should notify factory that symbol table has new
			 * entries, and may want to reuse this symbol table instead of
			 * current root.
			 */
			...
			/*
			 * Hmmh. Actually, we need to close all the dependant input sources,
			 * first, and then also call close() on the root input source
			 * object; it will only do real close if that was enabled earlier.
			 * The root input source also prevents multiple close() calls for
			 * the underlying source, so we need not check that here.
			 */
			closeAllInput(false);
	...
	public Object getFeature(String name) {
		throw new IllegalArgumentException(
				MessageFormat.format(ErrorConsts.ERR_UNKNOWN_FEATURE, new Object[] { name }));
	}
	 ...
	 *
	 * @return True, if the specified property was <b>succesfully</b> set to
	 *         specified value; false if its value was not changed
	 */
	@Override
	public boolean setProperty(String name, Object value) {
		boolean ok = mConfig.setProperty(name, value);
		/*
		 * To make [WSTX-50] work fully dynamically (i.e. allow setting BASE_URL
		 * after stream reader has been constructed) need to force
		 */
	...
	public AttributeInfo getAttributeInfo() throws XMLStreamException {
		...
		/*
		 * Although attribute collector knows about specific parsed information,
		 * the element stack has DTD-derived information (if any)... and knows
		 * how to call attribute collector when necessary.
		 */
		return mElementStack;
	}
	...
	/**
	 * Since this class implements {@link DTDInfo}, method can just return
	 * <code>this</code>.
	 */
	@Override
	public DTDInfo getDTDInfo() throws XMLStreamException {
		/*
		 * Let's not allow it to be accessed during other events -- that way
		 * callers won't count on it being available afterwards.
		 */
		if (mCurrToken != DTD) {
			return null;
		}
	...
	/**
	 * Method similar to {@link #getText()}, except that it just uses provided
	 * Writer to write all textual content. For further optimization, it may
	 * also be allowed to do true pass-through, thus possibly avoiding one
	 * temporary copy of the data.
	 * <p>
	 * TODO: try to optimize to allow completely streaming pass-through:
	 * currently will still read all data in memory buffers before outputting
	 * 
	 * @param w
	 *            Writer to use for writing textual contents
	 * @param preserveContents
	 *            If true, reader has to preserve contents so that further calls
	 *            to <code>getText</code> will return proper conntets. If false,
	 *            reader is allowed to skip creation of such copies: this can
	 *            improve performance, but it also means that further calls to
	 *            <code>getText</code> is not guaranteed to return meaningful
	 *            data.
	 *
	...
	public int getText(Writer w, boolean preserveContents) throws IOException, XMLStreamException {
		...
		/*
		 * May need to be able to do fully streaming... but only for text events
		 * that have not yet been fully read; for other types there's less
		 * benefit, and for fully read ones, we already have everything ready.
		 */
		if (!preserveContents) {
			if (currToken == CHARACTERS) {
				int count = mTextBuffer.rawContentsTo(w);
				/*
				 * Let's also clear whatever was collected (as allowed by method
				 * contract) previously, to both save memory, and to ensure
				 * caller doesn't accidentally try to access it (and get
				 * otherwise 'random' results).
				 */
				mTextBuffer.resetWithEmpty();
				if (mTokenState < TOKEN_FULL_SINGLE) {
					count += readAndWriteText(w);
				}
				if (mCfgCoalesceText && (mTokenState < TOKEN_FULL_COALESCED)) {
					if (mCfgCoalesceText) {
		...
		if (mTokenState < mStTextThreshold) {
			/*
			 * Otherwise, let's just finish the token; and due to guarantee by
			 * streaming method, let's try ensure we get it all.
			 */
	...
	public int getDepth() {
		/*
		 * Note: we can not necessarily use mCurrDepth, since it is directly
		 * synchronized to the input (to catch unbalanced entity expansion WRT
		 * element nesting), and not to actual token values returned.
		 */
	...
	/**
	 * @return Internal subset portion of the DOCTYPE declaration, if any; empty
	 *         String if none
	 */
	@Override
	...
	private char[] getDTDInternalSubsetArray() {
		/*
		 * Note: no checks for current state, but only because it's an internal
		 * method and callers are known to ensure it's ok to call this
		 */
	...
	public long getStartingByteOffset() {
		/*
		 * 15-Apr-2005, TSa: No way to reliably keep track of byte offsets, at
		 * least for variable-length encodings... so let's just return -1 for
		 * now
		 */
		return -1L;
	}
	...
	/**
	 * Method called by {@link com.ctc.wstx.evt.DefaultEventAllocator} to get
	 * double-indirection necessary for constructing start element events.
	 *
	...
	public Object withStartElement(ElemCallback cb, Location loc) {
		...
		return cb.withStartElement(loc, getName(), mElementStack.createNonTransientNsContext(loc),
	...
	/**
	 * Method needed by classes (like stream writer implementations) that want
	 * to have efficient direct access to element stack implementation
	 */
	@Override
	...
	/**
	 * Method needed by classes (like stream writer implementations) that want
	 * to have efficient direct access to attribute collector Object, for
	 * optimal attribute name and value access.
	 */
	@Override
	...
	/*
	 * /////////////////////////////////////////////////////////// // Support
	 * for SAX XMLReader implementation
	 * ///////////////////////////////////////////////////////////
	 */
	...
	public void fireSaxEndElement(ContentHandler h) throws SAXException {
		if (h != null) {
			/*
			 * Order of events is reversed (wrt. start-element): first the end
			 * tag event, then unbound prefixes
			 */
			String uri = mElementStack.getNsURI();
			// Sax requires "" (not null) for ns uris...
			h.endElement((uri == null) ? "" : uri, mElementStack.getLocalName(), getPrefixedName());
	...
	public void fireSaxCharacterEvents(ContentHandler h) throws XMLStreamException, SAXException {
		if (h != null) {
			...
			/*
			 * Let's not defer errors; SAXTest implies it's expected errors are
			 * thrown right away
			 */
			if (mTokenState < mStTextThreshold) {
				finishToken(false);
	...
	/*
	 * /////////////////////////////////////////////////////////// // Internal
	 * methods, parsing helper methods
	 * ///////////////////////////////////////////////////////////
	 */
	 ...
	 * {@link TextBuilder} instance. Will normalize white space inside attribute
	 * value using default XML rules (change linefeeds to spaces etc.; but won't
	 * use DTD information for further coalescing).
	 *
	 * @param openingQuote
	 *            Quote character (single or double quote) for this attribute
	 *            value
	 * @param tb
			...
			if (c <= '\'') {
				if (c < CHAR_SPACE) {
					if (c == '\n') {
						...
						/*
						 * 04-Mar-2006, TSa: Linefeed normalization only done if
						 * enabled -- specifically, 2-char lfs from int.
						 * entities are not coalesced. Now... whether to try to
						 * count them as one or not... easier not to; esp. since
						 * we may not be able to distinguish char entity
						 * originated ones from real ones.
						 */
						if (mNormalizeLFs) {
					 ...
					 * this will only be done at root level, there's no need to
					 * check for "runaway" values; they'll hit EOF
					 */
					if (mInput == currScope) {
						break;
					}
					...
					if (inputInBuffer() >= 3 && (ch = resolveSimpleEntity(true)) != 0) {
						...
						if (ch == 0) {
							// need to skip output, thusly (expanded to new
							// input source)
							continue;
						}
					}
	...
	/*
	 * /////////////////////////////////////////////////////////// // Internal
	 * methods, parsing prolog (before root) and // epilog
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * Method called to find type of next token in prolog; either reading just
	 * enough information to know the type (lazy parsing), or the full contents
	 * (non-lazy)
	 *
	...
	private boolean nextFromProlog(boolean isProlog) throws XMLStreamException {
		...
		if (i <= CHAR_SPACE && i >= 0) {
			...
			if (hasConfigFlags(CFG_REPORT_PROLOG_WS)) {
				...
				if (readSpacePrimary((char) i, true)) {
					...
					if (mCfgLazyParsing) {
						/*
						 * Let's not even bother checking if it's "long enough";
						 * shouldn't usually matter, but few apps care to get
						 * multiple adjacent SPACE events...
						 */
			...
			if (i >= 0) {
				...
				/*
				 * 11-Apr-2005, TSa: But note that we need to "move back" column
				 * and total offset values by one, to compensate for the char
				 * that was read (row can not have changed, since it's non-WS,
				 * and thus non-lf/cr char)
				 */
		...
		if (i != '<') {
			throwUnexpectedChar(i, (isProlog ? SUFFIX_IN_PROLOG : SUFFIX_IN_EPILOG) + "; expected '<'");
		}
		...
		if (c == '?') { // proc. inst
		...
		} else if (c == '!') { // DOCTYPE or comment (or CDATA, but not legal
								// here)
			...
			if (!isProlog) {
				 ...
				 */
				mCurrToken = handleExtraRoot(c); // will check input parsing
													// mode...
				return false;
			}
			handleRootElem(c);
			...
			throwUnexpectedChar(c, (isProlog ? SUFFIX_IN_PROLOG : SUFFIX_IN_EPILOG) + ", after '<'.");
		}
		...
		if (mRootLName != null) {
			if (hasConfigFlags(CFG_VALIDATE_AGAINST_DTD)) {
				if (!mElementStack.matches(mRootPrefix, mRootLName)) {
					String actual = (mRootPrefix == null) ? mRootLName : (mRootPrefix + ":" + mRootLName);
	...
	/**
	 * Method called right before the document root element is handled. The
	 * default implementation is empty; validating stream readers should
	 * override the method and do whatever initialization is necessary
	 */
	...
	protected int handleEOF(boolean isProlog) throws XMLStreamException {
		...
		/*
		 * Although buffers have most likely already been recycled, let's call
		 * this again just in case. At this point we can safely discard any
		 * contents
		 */
	...
	/**
	 * Method called if a root-level element is found after the main root
	 * element was closed. This is legal in multi-doc parsing mode (and in
	 * fragment mode), but not in the default single-doc mode.
	 * 
	 * @param c
	 *            Character passed in (not currently used)
	 *
	...
	private int handleExtraRoot(char c) throws XMLStreamException {
		if (!mConfig.inputParsingModeDocuments()) {
			/*
			 * Has to be single-doc mode, since fragment mode should never get
			 * here (since fragment mode never has epilog or prolog modes)
			 */
	 ...
	 * END_DOCUMENT/START_DOCUMENT event combination, followed by the handling
	...
	protected int handleMultiDocStart(int nextEvent) {
		...
		mTokenState = TOKEN_FULL_COALESCED; // this is a virtual event after
											// all...
	...
	private int nextFromMultiDocState() throws XMLStreamException {
		if (mCurrToken == END_DOCUMENT) {
			/*
			 * Ok; this is the initial step; need to advance: need to parse xml
			 * declaration if that was the cause, otherwise just clear up
			 * values.
			 */
			if (mSecondaryToken == START_DOCUMENT) {
				handleMultiDocXmlDecl();
			} else { // Nah, DOCTYPE or start element... just need to clear decl
						// info:
		...
		if (mCurrToken == START_DOCUMENT) {
			mParseState = STATE_PROLOG; // yup, we are now officially in prolog
										// again...
		...
		throw new IllegalStateException("Internal error: unexpected state; current event " + tokenTypeDesc(mCurrToken)
				+ ", sec. state: " + tokenTypeDesc(mSecondaryToken));
	}
	...
	protected void handleMultiDocXmlDecl() throws XMLStreamException {
		...
		if (tb.equalsString(XmlConsts.XML_V_10_STR)) {
			...
			mDocXmlVersion = XmlConsts.XML_V_UNKNOWN;
			mXml11 = false;
			throwParseError("Unexpected xml version '" + tb.toString() + "'; expected '" + XmlConsts.XML_V_10_STR
		...
		if (c != '?') { // '?' signals end...
			if (c == 'e') { // encoding
				...
				throwUnexpectedChar(c,
						" in xml declaration; expected either 'encoding' or 'standalone' pseudo-attribute");
			}
			...
			if (c == 's') {
				...
				if (tb.equalsString(XmlConsts.XML_SA_YES)) {
					...
					throwParseError("Unexpected xml '" + XmlConsts.XML_DECL_KW_STANDALONE + "' pseudo-attribute value '"
							+ tb.toString() + "'; expected '" + XmlConsts.XML_SA_YES + "' or '" + XmlConsts.XML_SA_NO
							+ "'");
				}
				c = getNextInCurrAfterWS(SUFFIX_IN_XML_DECL);
			}
	...
	/**
	 * Method that checks that input following is of form '[S]* '=' [S]*' (as
	 * per XML specs, production #25). Will push back non-white space characters
	 * as necessary, in case no equals char is encountered.
	 */
	 ...
	 * included, and in general need not be as picky (since caller is to verify
	 * contents). One exception is that we do check for linefeeds and lt chars,
	 * since they generally would indicate problems and are useful to catch
	 * early on (can happen if a quote is missed etc)
	 * <p>
	 * Note: since it'll be called at most 3 times per document, this method is
	...
	protected final void parseQuoted(String name, char quoteChar, TextBuffer tbuf) throws XMLStreamException {
		if (quoteChar != '"' && quoteChar != '\'') {
			throwUnexpectedChar(quoteChar,
					" in xml declaration; waited ' or \" to start a value for pseudo-attribute '" + name + "'");
		}
		char[] outBuf = tbuf.getCurrentSegment();
		int outPtr = 0;

		while (true) {
			char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_XML_DECL);
	 ...
	 * that it'll either be DOCTYPE declaration (if we are in prolog and haven't
	 * yet seen one), or a comment. CDATA is not legal here; it would start same
	 * way otherwise.
	 */
	...
	private void startDTD() throws XMLStreamException {
		/*
		 * 21-Nov-2004, TSa: Let's make sure that the buffer gets cleared at
		 * this point. Need not start branching yet, however, since DTD event is
		 * often skipped.
		 */
		mTextBuffer.resetInitialized();

		/*
		 * So, what we need is:<code> <!DOCTYPE' S Name (S ExternalID)? S? ('['
		 * intSubset ']' S?)? '> </code>. And we have already read the DOCTYPE
		 * token.
		 */
			...
			if (c == 'P') {
				...
				if (keyw != null) {
					...
					if (!skipWS(getNextChar(SUFFIX_IN_DTD))) {
						throwUnexpectedChar(c,
								SUFFIX_IN_DTD + "; expected a space between PUBLIC keyword and public id");
					}
					...
					if (!skipWS(getNextChar(SUFFIX_IN_DTD))) {
						throwUnexpectedChar(c,
								SUFFIX_IN_DTD + "; expected a space between public and system identifiers");
					}
					c = getNextCharFromCurrent(SUFFIX_IN_DTD);
					if (c != '"' && c != '\'') {
				...
				if (!isNameStartChar(c)) {
					...
					--mInputPtr;
					keyw = checkKeyword(c, "SYSTEM"); // keyword passed in
														// doesn't matter
				}
			}
		...
		/*
		 * Actually, let's just push whatever char it is, back; this way we can
		 * lazily initialize text buffer with DOCTYPE declaration if/as
		 * necessary, even if there's no internal subset.
		 */
		--mInputPtr; // pushback
		mTokenState = TOKEN_STARTED;
	}
	 ...
	 * essentially the optional internal subset. This class implements the basic
	 * "ignore it" functionality, but can optionally still store copy of the
	 * contents to the read buffer.
	 * <p>
	 * NOTE: Since this default implementation will be overridden by some
	 * sub-classes, make sure you do NOT change the method signature.
	 *
	 * @param copyContents
	 *            If true, will copy contents of the internal subset of DOCTYPE
	 *            declaration in the text buffer; if false, will just completely
	 *            ignore the subset (if one found).
	 */
	protected void finishDTD(boolean copyContents) throws XMLStreamException {
		/*
		 * We know there are no spaces, as this char was read and pushed back
		 * earlier...
		 */
		...
		if (c == '[') {
			...
			try {
				...
				if (copyContents) {
					/*
					 * Need to "push back" ']' got in the succesful case (that's
					 * -1 part below); in error case it'll just be whatever last
					 * char was.
					 */
	...
	/*
	 * /////////////////////////////////////////////////////////// // Internal
	 * methods, main parsing (inside root)
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * Method called to parse beginning of the next event within document tree,
	 * and return its type.
	 */
	private final int nextFromTree() throws XMLStreamException {
		...
		if (mTokenState < mStTextThreshold) {
			...
			/*
			 * 03-Mar-2006, TSa: Let's add a sanity check here, temporarily, to
			 * ensure we never skip any textual content when it is to be
			 * validated
			 */
			if (mVldContent == XMLValidator.CONTENT_ALLOW_VALIDATABLE_TEXT) {
				if (mCurrToken == CHARACTERS || mCurrToken == CDATA) { // should
																		// never
																		// happen
			...
			if (mCurrToken == START_ELEMENT) {
				...
				if (mStEmptyElem) {
					...
					// 27-Feb-2009, TSa: but we do have to handle validation of
					// the end tag now
				...
				/*
				 * Just returned a partial CDATA... that's ok, just need to know
				 * we won't get opening marker etc. The tricky part here is just
				 * to ensure there's at least one character; if not, need to
				 * just discard the empty 'event' (note that it is possible to
				 * have an initial empty CDATA event for truly empty CDATA
				 * block; but not partial ones!). Let's just read it like a new
				 * CData section first:
				 */
				...
				mTokenInputRow = mCurrInputRow;
				mTokenInputCol = mInputPtr - mCurrInputRowStart;
				char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_CDATA);
				if (readCDataPrimary(c)) { // got it all!
					...
					if (mTextBuffer.size() == 0 && readCDataSecondary(mCfgLazyParsing ? 1 : mShortestTextSegment)) {
						// Ok, all of it read
						if (mTextBuffer.size() > 0) {
				...
				/*
				 * If we get here, it was the end of the section, without any
				 * more text inside CDATA, so let's just continue
				 */
			}
		...
		/*
		 * 26-Aug-2004, TSa: We have to deal with entities, usually, if they are
		 * the next thing; even in non-expanding mode there are entities and
		 * then there are entities... :-) Let's start with char entities; they
		 * can be expanded right away.
		 */
		while (i == '&') {
			mWsStatus = ALL_WS_UNKNOWN;

			/*
			 * 30-Aug-2004, TSa: In some contexts entities are not allowed in
			 ...
			 */
			int ch = mCfgReplaceEntities ? fullyResolveEntity(true) : resolveCharOnlyEntity(true);
			...
			if (ch != 0) {
				...
				if (mVldContent <= XMLValidator.CONTENT_ALLOW_WS) {
					...
					if (ch > CHAR_SPACE) {
						 ...
						 * 'mElementStack.reallyValidating', if only ws allowed,
						 * to cover the case where non-typing-dtd was only used
						 * to discover SPACE type. But now that we have
						 * CONTENT_ALLOW_WS_NONSTRICT, shouldn't be needed.
						 */
						// if (mVldContent < XMLValidator.CONTENT_ALLOW_WS ||
			...
			if (!mCfgReplaceEntities || mCfgTreatCharRefsAsEntities) {
				...
				mTokenState = TOKEN_FULL_COALESCED;
				/*
				 * // let's not worry about non-parsed entities, since this is
				 * unexpanded mode // ... although it'd be an error either way?
				 * Should we report it? if (ed != null && !ed.isParsed()) {
				 * throwParseError("Reference to unparsed entity '"+ed.getName()
		 ...
		 */
		/*
		 * But first, do we expect to get ignorable white space (only happens in
		 * validating mode)? If so, needs bit different handling:
		 */
		if (mVldContent <= XMLValidator.CONTENT_ALLOW_WS_NONSTRICT) {
			...
			if (i <= CHAR_SPACE) {
				 ...
				 */
				mTokenState = (readSpacePrimary((char) i, false)) ? TOKEN_FULL_COALESCED : TOKEN_STARTED;
				return SPACE;
			}
			...
			/*
			 * otherwise, we know it's supposed to contain just space (or be
			 * empty), but as we are not validating it's not an error for this
			 * not to be true. Type should be changed to CHARACTERS tho.
			 */
		}
		...
		if (readTextPrimary((char) i)) { // reached following markup
			...
			if (!mCfgCoalesceText && mTextBuffer.size() >= mShortestTextSegment) {
				mTokenState = TOKEN_PARTIAL_SINGLE;
			} else {
				mTokenState = TOKEN_STARTED;
			}
	...
	/**
	 * Method called when advacing stream past the end tag that closes the root
	 * element of the open document. Document can be either the singular one, in
	 * regular mode, or one of possibly multiple, in multi-doc mode: this method
	 * is never called in fragment mode. Method needs to update state properly
	 * and parse following epilog event (if any).
	 *
	...
	private int closeContentTree() throws XMLStreamException {
		...
		/*
		 * 10-Apr-2006, TSa: Let's actually try to update SymbolTable here
		 * (after main xml tree); caller may not continue parsing after this.
		 */
		if (mSymbols.isDirty()) {
			mOwner.updateSymbolTable(mSymbols);
		}
	...
	/**
	 * Method that takes care of parsing of start elements; including full
	 * parsing of namespace declarations and attributes, as well as namespace
	 * resolution.
	 */
	private final void handleStartElem(char c) throws XMLStreamException {
		...
		if (mCfgNsEnabled) {
			String str = parseLocalName(c);
			c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_EOF_EXP_NAME);
			if (c == ':') { // Ok, got namespace and local name
				c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_EOF_EXP_NAME);
			...
			/*
			 * Enough about element name itself; let's then parse attributes and
			 * namespace declarations. Split into another method for clarity,
		...
		/*
		 * 27-Feb-2009, TSa: [WSTX-191]: We used to validate virtual end element
		 * here for empty elements, but it really should occur later on when
		 * actually returning that end element.
		 */
...
//				tell the attribute listener, that a attribute name starts here
			...
			/*
			 * 19-Jul-2004, TSa: Need to check that non-default namespace URI is
			 * NOT empty, as per XML namespace specs, #2, ("...In such
			 ...
			 */
			/*
			 * 04-Feb-2005, TSa: Namespaces 1.1 does allow this, though, so for
			 * xml 1.1 documents we need to allow it
			 */
			if (!mXml11) {
				if (startLen >= 0 && tb.getCharSize() == startLen) { // is
																		// empty!
	...
	private static final int ATTR_LIST_REGION_START = 0;
	private static final int ATTR_LIST_NAME_START = 1;
	private static final int ATTR_LIST_NAME_END = 2;
	private static final int ATTR_LIST_VALUE_START = 3;
	private static final int ATTR_LIST_VALUE_END = 4;
	private static final int ATTR_LIST_REGION_END = 5;
...
//				tell the attribute listener, that a attribute name starts here
	...
	/**
	 * Method called to completely read a close tag, and update element stack
	 * appropriately (including checking that tag matches etc).
	 */
	protected final void readEndElem() throws XMLStreamException {
		...
		char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_IN_CLOSE_ELEMENT);
		...
		/*
		 * Ok, now; good thing is we know exactly what to compare against...
		 */
		...
		if (expPrefix != null && expPrefix.length() > 0) {
			...
			c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_IN_CLOSE_ELEMENT);
			if (c != ':') {
				reportWrongEndPrefix(expPrefix, expLocalName, i);
				return;
			}
		...
		/*
		 * 13-Feb-2006, TSa: Are we about to close an element that started
		 * within a parent element? That's a GE/element nesting WFC violation...
		 */
	...
	private void reportWrongEndPrefix(String prefix, String localName, int done) throws XMLStreamException {
		...
		throwParseError("Unexpected close tag </" + actName + ">; expected </" + fullName + ">.");
	}
	 ...
	 * returned as CHARACTERS, never as CDATA. And since at this point we don't
	 * really know if there's anything to coalesce (but there may be), let's
	 * convert CDATA if necessary.
	 */
	private int nextFromTreeCommentOrCData() throws XMLStreamException {
		...
		if (c == '[') {
			checkCData();
			/*
			 * Good enough; it is a CDATA section... but let's just also parse
			 * the easy ("free") stuff:
			 */
			c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_IN_CDATA);
	...
	/**
	 * Method called to skip last part of current token, when full token has not
	 * been parsed. Generally happens when caller is not interested in current
	 * token and just calls next() to iterate to next token.
	 * <p>
	 * Note: this method is to accurately update the location information to
	 * reflect where the next event will start (or, in case of EOF, where EOF
	 * was encountered, ie. where event would start, if there was one).
	 *
	 * @return Next character after node has been skipped, or -1 if EOF follows
	 */
	private int skipToken() throws XMLStreamException {
		int result;

		main_switch: switch (mCurrToken) {
		case CDATA: {
			/*
			 * 30-Aug-2004, TSa: Need to be careful here: we may actually have
			 * finished with CDATA, but are just coalescing... if so, need to
			 * skip first part of skipping
			 */
		 ...
		 *
		 * + 0 -> could reliably read the prev event, now need the following
		 * char/EOF + -1 -> hit EOF; can return it + something else -> this is
		 * the next char, return it.
		 *
		 * In first 2 cases, next event start offset is the current location; in
		 * third case, it needs to be backtracked by one char
		 */
		if (result < 1) {
			mTokenInputRow = mCurrInputRow;
			mTokenInputTotal = mCurrInputProcessed + mInputPtr;
	...
	private void skipCommentOrCData(String errorMsg, char endChar, boolean preventDoubles) throws XMLStreamException {
		/*
		 * Let's skip all chars except for double-ending chars in question
		 * (hyphen for comments, right brack for cdata)
		 */
		int count = 0;
		while (true) {
			char c;
			...
			if (c == endChar) { // Probably?
				...
				while (c == endChar) {
					c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(errorMsg);
				}
				if (c == '>') {
					break;
				}
	...
	/**
	 * Method called to skip past all following text and CDATA segments, until
	 * encountering something else (including a general entity, which may in
	 * turn expand to text).
	 *
	 * @return Character following all the skipped text and CDATA segments, if
	 *         any; or -1 to denote EOF
	 */
	private int skipCoalescedText(int i) throws XMLStreamException {
		while (true) {
			...
			if (i == '<') { // markup, maybe CDATA?
				...
				if (!ensureInput(3)) {
					/*
					 * Most likely an error condition, but let's leave it up for
					 * other parts of code to complain.
					 */
					return i;
				}
				if (mInputBuffer[mInputPtr] != '!' || mInputBuffer[mInputPtr + 1] != '[') {
				...
				i = skipTokenText(i);
				/*
				 * Did we hit an unexpandable entity? If so, need to return
				 * ampersand to the caller... (and same for EOF too)
				 */
				if (i == '&' || i < 0) {
					return i;
				}
	...
	private int skipTokenText(int i) throws XMLStreamException {
		/*
		 * Fairly easy; except for potential to have entities expand to some
		 * crap?
		 */
		int count = 0;
		...
		main_loop: while (true) {
			...
			if (i == '&') {
				...
				if (mCfgReplaceEntities) {
					...
					if ((mInputEnd - mInputPtr) >= 3 && resolveSimpleEntity(true) != 0) {
						;
					} else {
					...
					/*
					 * Can only skip character entities; others need to be
					 * returned separately.
					 */
	...
	protected void safeFinishToken() {
		try {
			 ...
			 * (wrapped) exceptions we throw, and defer some. For now, this is
			 * only for CHARACTERS (since it's always legal to split CHARACTERS
			 * segment); could be expanded in future.
			 */
	...
	/**
	 * Method called to read in contents of the token completely, if not yet
	 * read. Generally called when caller needs to access anything other than
	 * basic token type (except for elements), text contents or such.
	 *
	 * @param deferErrors
	 *            Flag to enable storing an exception to a variable, instead of
	...
	protected void finishToken(boolean deferErrors) throws XMLStreamException {
		...
		case CHARACTERS:
			if (mCfgCoalesceText) {
				/*
				 * 21-Sep-2005, TSa: It is often possible to optimize here: if
				 * we get '<' NOT followed by '!', it can not be CDATA, and thus
				 * we are done.
				 */
				if (mTokenState == TOKEN_FULL_SINGLE && (mInputPtr + 1) < mInputEnd
						&& mInputBuffer[mInputPtr + 1] != '!') {
					mTokenState = TOKEN_FULL_COALESCED;
					return;
				}
		...
		case SPACE: {
			/*
			 * Only need to ensure there's no non-whitespace text when parsing
			 * 'real' ignorable white space (in validating mode, but that's
			 * implicit here)
			 */
		...
		case DTD:
			...
			/*
			 * 05-Jan-2006, TSa: Although we shouldn't have to use finally here,
			 * it's probably better to do that for robustness (specifically, in
			 * case of a parsing problem, we don't want to remain in 'DTD
			 * partially read' case -- it's better to get in panic mode and skip
			 * the rest)
			 */
			try {
				finishDTD(true);
	...
	private void readComment() throws XMLStreamException {
		...
		while (ptr < inputLen) {
			...
			if (c < CHAR_SPACE) {
				...
				if ((ptr + 1) >= inputLen) {
					/*
					 * Can't check next 2, let's push '-' back, for rest of code
					 * to take care of
					 */
					--ptr;
					break;
	...
	private void readComment2(TextBuffer tb) throws XMLStreamException {
		/*
		 * Output pointers; calls will also ensure that the buffer is not
		 * shared, AND has room for at least one more char
		 */
		...
		while (true) {
			char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_IN_COMMENT);
			...
			if (c < CHAR_SPACE) {
				...
				/*
				 * Not the end marker; let's just output the first hyphen, push
				 * the second char back , and let main code handle it.
				 */
				c = '-';
				--mInputPtr;
			}
	...
	/**
	 * Method that reads the primary part of a PI, ie. target, and also skips
	 * white space between target and data (if any data)
	 *
	 * @return Usually <code>PROCESSING_INSTRUCTION</code>; but may be different
	 *         in multi-doc mode, if we actually hit a secondary xml
	 *         declaration.
	 */
	private final int readPIPrimary() throws XMLStreamException {
		...
		char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextCharFromCurrent(SUFFIX_IN_PROC_INSTR);
	...
	/**
	 * Method that parses a processing instruction's data portion; at this point
	 * target has been parsed.
	 */
			...
			if (c < CHAR_SPACE) {
				...
				while (true) {
					if (ptr >= inputLen) {
						 ...
						 * question marks (not all, since just one is needed to
						 * close the PI)
						 */
						--ptr;
						break outer_loop;
					...
					if (c != '?') {
						// Not end, can continue, but need to push back last
						// char, in case it's LF/CR
						--ptr;
						break;
					}
				}
	...
	private void readPI2(TextBuffer tb) throws XMLStreamException {
		...
		/*
		 * Output pointers; calls will also ensure that the buffer is not
		 * shared, AND has room for one more char
		 */
		char[] outBuf = tb.getCurrentSegment();
		int outPtr = tb.getCurrentSegmentSize();
			...
			if (c < CHAR_SPACE) {
				...
				qmLoop: while (true) {
					...
					if (c == '>') { // got it!
						...
						/*
						 * Hmmh. Wasn't end mark after all. Thus, need to fall
						 * back to normal processing, with one more question
						 * mark (first one matched that wasn't yet output),
	 ...
	 * events, and all following consequtive events into the text buffer. At
	 * this point the current type is known, prefix (for CDATA) skipped, and
	 * initial consequtive contents (if any) read in.
	 *
	 * @param deferErrors
	 *            Flag to enable storing an exception to a variable, instead of
	 *            immediately throwing it. If true, will just store the
	...
	protected void readCoalescedText(int currType, boolean deferErrors) throws XMLStreamException {
		...
		if (currType == CHARACTERS || currType == SPACE) {
			...
			/*
			 * We may have actually really finished it, but just left the
			 * 'unfinished' flag due to need to coalesce...
			 */
			if (mTokenState <= TOKEN_PARTIAL_SINGLE) {
			...
			throw new IllegalStateException("Internal error: unexpected token " + tokenTypeDesc(mCurrToken)
					+ "; expected CHARACTERS, CDATA or SPACE.");
		}
		...
		while (!deferErrors || (mPendingException == null)) {
			...
			if (c == '<') { // CDATA, maybe?
				...
				if ((mInputEnd - mInputPtr) < 9) { // 3 for "<![" and 6 more for
													// "CDATA["
					mTextBuffer.ensureNotShared();
					if (!ensureInput(3)) {
						break;
				...
				if (mInputBuffer[mInputPtr + 1] != '!' || mInputBuffer[mInputPtr + 2] != '[') {
					// Nah, some other tag or directive
					break;
				}
				// Let's skip beginning parts, then:
				...
				checkCData();
				/*
				 * No need to call the primary data; it's only useful if there's
				 * a chance for sharing buffers... so let's call the secondary
				 * loop straight on.
				 */
				readCDataSecondary(Integer.MAX_VALUE);
				wasCData = true;
				...
				/*
				 * Did we hit an 'unexpandable' entity? If so, need to just bail
				 * out.
				 */
				if (c == '&' && !wasCData) {
					break;
	...
	/**
	 * Method called to read in consecutive beginning parts of a CDATA segment,
	 * up to either end of the segment (]] and >) or until first 'hole' in text
	 * (buffer end, 2-char lf to convert, entity).
	 * <p>
	 * When the method is called, it's expected that the first character has
	 * been read as is in the current input buffer just before current pointer
	 *
	 * @param c
	 *            First character in the CDATA segment (possibly part of end
	 *            marker for empty segments
	 *
	 * @return True if the whole CDATA segment was completely read; this happens
	 *         only if lt-char is hit; false if it's possible that it wasn't
	 *         read (ie. end-of-buffer or entity encountered).
	 */
	private final boolean readCDataPrimary(char c) throws XMLStreamException {
		...
		while (true) {
			if (c < CHAR_SPACE) {
				...
				if ((ptr + 1) >= inputLen) { // not enough room? need to push it
												// back
					--ptr;
					break;
				}
		...
		/*
		 * If we end up here, we either ran out of input, or hit something which
		 * would leave 'holes' in buffer... fine, let's return then; we can
		 * still update shared buffer copy: would be too early to make a copy
		 * since caller may not even be interested in the stuff.
		 */
		int len = ptr - start;
	...
	/**
	 * @return True if the whole CData section was completely read (we hit the
	 *         end marker); false if a shorter segment was returned.
	 */
	protected boolean readCDataSecondary(int shortestSegment) throws XMLStreamException {
		...
		while (true) {
			...
			if (outPtr >= outBuf.length) {
				...
				outBuf = tb.finishCurrentSegment();
				outPtr = 0;
				// 17-Aug-2016, tatu: need to make sure to enforce size limits
				// here too
				verifyLimit("Text size", mConfig.getMaxTextLength(), mTextBuffer.size());
			}
		}
	...
	/**
	 * Method that will check, given the starting ']', whether there is ending
	 * ']]>' (including optional extra ']'s); if so, will updated output buffer
	 * with extra ]s, if not, will make sure input and output are positioned for
	 * further checking.
	 * 
	...
	private boolean checkCDataEnd(char[] outBuf, int outPtr) throws XMLStreamException {
		...
		while (bracketCount > 0) {
			...
			if (outPtr >= outBuf.length) {
				/*
				 * Can't really easily return, even if we have enough stuff
				 * here, since we've more than one char...
				 */
				outBuf = mTextBuffer.finishCurrentSegment();
				outPtr = 0;
			}
	...
	/**
	 * Method called to read in consecutive beginning parts of a text segment,
	 * up to either end of the segment (lt char) or until first 'hole' in text
	 * (buffer end, 2-char lf to convert, entity).
	 * <p>
	 * When the method is called, it's expected that the first character has
	 * been read as is in the current input buffer just before current pointer
	 *
	 * @param c
	 *            First character of the text segment
	 *
	 * @return True if the whole text segment was completely read; this happens
	 *         only if lt-char is hit; false if it's possible that it wasn't
	 *         read (ie. end-of-buffer or entity encountered).
	 */
	private final boolean readTextPrimary(char c) throws XMLStreamException {
		...
		if (c <= CHAR_SPACE) {
			int len = mInputEnd;
			/*
			 * Even without indentation removal, it's good idea to 'convert' \r
			 * or \r\n into \n (by replacing or skipping first char): this may
			 * allow reusing the buffer. But note that conversion MUST be
			 * enabled -- this is toggled by code that includes internal
			 * entities, to prevent replacement of CRs from int. general
				...
				if (ptr < len && mNormalizeLFs) {
					if (c == '\r') {
						...
						if (mInputBuffer[ptr] == c) {
							/*
							 * Ok, whatever happens, can 'skip' \r, to point to
							 * following \n:
							 */
							++start;
					...
					// If we got this far, we skipped a lf, need to read next
					// char
					c = mInputBuffer[ptr++];
				}
			} while (false);
			...
			if (c < CHAR_FIRST_PURE_TEXT) {
				...
				if (c < CHAR_SPACE) {
					if (c == '\n') {
						...
						if (mNormalizeLFs) { // can we do in-place Mac
												// replacement?
							if (inputBuf[ptr] == '\n') { // nope, 2 char lf
								--ptr;
								break;
						...
						// Should consume invalid char, but not include in
						// result
						mInputPtr = ptr;
						mTextBuffer.resetWithShared(inputBuf, start, ptr - start - 1);
						/*
						 * Let's defer exception, provided we got at least one
						 * valid character (if not, better throw exception right
						 * away)
						 */
	...
	protected final boolean readTextSecondary(int shortestSegment, boolean deferErrors) throws XMLStreamException {
		...
		while (true) {
			if (inputPtr >= inputLen) {
				 ...
				 * other modes we might as well first return text, and only then
				 * throw an exception: no need to do that yet.
				 */
				mInputPtr = inputPtr;
				if (!loadMore()) {
					break;
			...
			if (c < CHAR_FIRST_PURE_TEXT) {
				if (c < CHAR_SPACE) {
					if (c == '\n') {
						...
						if (skipCRLF(c)) { // got 2 char LF
							if (!mNormalizeLFs) {
								// Special handling, to output 2 chars at a
								// time:
								outBuf[outPtr++] = c;
								if (outPtr >= outBuf.length) { // need more
																// room?
									outBuf = mTextBuffer.finishCurrentSegment();
									outPtr = 0;
								}
							...
							// And let's let default output the 2nd char
							c = '\n';
						} else if (mNormalizeLFs) { // just \r, but need to
													// convert
							c = '\n'; // For Mac text
						}
						/*
						 * note: skipCRLF() may change ptr and len, but since it
						 * does not close input source, it won't change actual
						 * buffer object:
						 */
						// inputBuffer = mInputBuffer;
						inputLen = mInputEnd;
					...
					if (mCfgReplaceEntities) { // can we expand all entities?
						if ((inputLen - inputPtr) >= 3 && (ch = resolveSimpleEntity(true)) != 0) {
							// Ok, it's fine then
						} else {
							ch = fullyResolveEntity(true);
							if (ch == 0) {
								// Input buffer changed, nothing to output quite
								// yet:
								inputBuffer = mInputBuffer;
								inputLen = mInputEnd;
								inputPtr = mInputPtr;
						...
						/*
						 * Nope, can only expand char entities; others need to
					...
					if (ch <= 0xFFFF) {
						...
						outBuf[outPtr++] = (char) ((ch >> 10) + 0xD800);
						if (outPtr >= outBuf.length) {
							if ((outBuf = _expandOutputForText(inputPtr, outBuf, Integer.MAX_VALUE)) == null) { // got
																												// enough,
																												// leave
								return false;
							}
					 ...
					 * as it contains _expanded_ stuff... only input side. For
					 * now, 98% accuracy has to do, as we may not be able to
					 * access previous buffer's contents. But at least we won't
					 * produce false positives from entity expansion
					 */
					if (inputPtr > 2) { // can we do it here?
						// Since mInputPtr has been advanced, -1 refers to '>'
						if (inputBuffer[inputPtr - 3] == ']' && inputBuffer[inputPtr - 2] == ']') {
							mInputPtr = inputPtr;
							/*
						...
						/*
						 * 21-Apr-2005, TSa: No good way to verify it, at this
						 * point. Should come back and think of how to properly
						 * handle this (rare) possibility.
						 */
						;
					}
				}
			...
			// Need more room?
			if (outPtr >= outBuf.length) {
				if ((outBuf = _expandOutputForText(inputPtr, outBuf, shortestSegment)) == null) { // got
																									// enough,
																									// leave
					return false;
				}
	...
	/**
	 * Method called to try to parse and canonicalize white space that has a
	 * good chance of being white space with somewhat regular structure;
	 * specifically, something that looks like typical indentation.
	 * <p>
	 * Note: Caller guarantees that there will be at least 2 characters
	 * available in the input buffer. And method has to ensure that if it does
	 * not find a match, it will return pointer value such that there is at
	 * least one valid character remaining.
	 *
	 * @return -1, if the content was determined to be canonicalizable
	 *         (indentation) white space; and thus fully parsed. Otherwise
	 *         pointer (value to set to mInputPtr) to the next character to
	 *         process (not processed by this method)
	 */
	private final int readIndentation(char c, int ptr) throws XMLStreamException {
		/*
		 * We need to verify that: (a) we can read enough contiguous data to do
		 * determination (b) sequence is a linefeed, with either zero or more
		 * following spaces, or zero or more tabs; and followed by non-directive
		 * tag (start/end tag) and if so, we can use a canonical shared
		 * representation of this even.
		 */
		...
		ws_loop: do { // dummy loop to allow for break (which indicates failure)
			c = inputBuf[ptr++];
			if (c == ' ' || c == '\t') { // indentation?
				...
				--ptr; // simpler if we just push it back; needs to be processed
						// later on
				break ws_loop;
			}
		...
		--mCheckIndentation;
		/*
		 * Also; if lf we got was \r, need to convert it now (this method only
		 * gets called in lf converting mode) (and yes, it is safe to modify
		 * input buffer at this point; see calling method for details)
		 */
		if (lf == '\r') {
			inputBuf[start] = '\n';
		}
	 ...
	 *
	 * @param c
	 *            First white space characters; known to contain white space at
	 *            this point
	 * @param prologWS
	 *            If true, is reading white space outside XML tree, and as such
	 *            can get EOF. If false, should not get EOF, nor be followed by
	 *            any other char than &lt;
	 *
	...
	private final boolean readSpacePrimary(char c, boolean prologWS) throws XMLStreamException {
		...
		while (true) {
			/*
			 * 30-Aug-2006, TSa: Let's not check for validity errors yet, even
			 * if we could detect problems at this point. This because it's not
			 * always an error (in dtd-aware, non-validating mode); but also
			 * since that way we can first return all space we got, and only
		...
		/*
		 * Ok, couldn't read it completely, let's just return whatever we did
		 * get as shared data
		 */
		mTextBuffer.resetWithShared(inputBuf, start, ptr - start);
		return false;
	...
	/**
	 * This is very similar to readSecondaryText(); called when we need to read
	 * in rest of (ignorable) white space segment.
	 *
	 * @param prologWS
	 *            True if the ignorable white space is within prolog (or
	 *            epilog); false if it's within xml tree.
	 */
	private void readSpaceSecondary(boolean prologWS) throws XMLStreamException {
		...
		while (true) {
			if (mInputPtr >= mInputEnd) {
				/*
				 * 07-Oct-2005, TSa: Let's not throw an exception yet -- can
				 * return SPACE, and let exception be thrown when trying to
				 * fetch next event.
				 */
				if (!loadMore()) {
					break;
	...
	/**
	 * Method called to read the contents of the current CHARACTERS event, and
	 * write all contents using the specified Writer.
	 *
	 * @param w
	 *            Writer to use for writing out textual content parsed
	 *
	...
	private int readAndWriteText(Writer w) throws IOException, XMLStreamException {
		...
		/*
		 * We should be able to mostly just use the input buffer at this point;
		 * exceptions being two-char linefeeds (when converting to single ones)
		 * and entities (which likewise can expand or shrink), both of which
		 * require flushing and/or single byte output.
		 */
		int start = mInputPtr;
		int count = 0;
		...
		main_loop: while (true) {
			...
			if (c < CHAR_FIRST_PURE_TEXT) {
				if (c < CHAR_SPACE) {
					...
					/*
					 * Have to flush all stuff, since entities pretty much force
					 * it; input buffer won't be contiguous
					 */
					...
					int ch;
					if (mCfgReplaceEntities) { // can we expand all entities?
						if ((mInputEnd - mInputPtr) < 3 || (ch = resolveSimpleEntity(true)) == 0) {
							ch = fullyResolveEntity(true);
						}
					} else {
						ch = resolveCharOnlyEntity(true);
						if (ch == 0) { // some other entity...
							 ...
							 * let's also ensure no text is added twice, as all
							 * prev text was just flushed, but resolve may have
							 * moved input buffer around.
							 */
							start = mInputPtr;
							break main_loop;
						}
					...
					if (mInputPtr >= 2) { // can we do it here?
						if (mInputBuffer[mInputPtr - 2] == ']' && mInputBuffer[mInputPtr - 1] == ']') {
							// Anything to flush?
							int len = mInputPtr - start;
							if (len > 0) {
								w.write(mInputBuffer, start, len);
	 ...
	 * read) CDATA event, and write all contents using the specified Writer.
	 *
	 * @param w
	 *            Writer to use for writing out textual content parsed
	 *
	 * @return Total number of characters written using the writer for the
	 *         current CDATA event
	 */
	private int readAndWriteCData(Writer w) throws IOException, XMLStreamException {
		...
		/*
		 * Ok; here we can basically have 2 modes; first the big loop to gather
		 * all data up until a ']'; and then another loop to see if ']' is part
		 * of ']]>', and after this if no end marker found, go back to the first
		 * part.
		 */
		char c = (mInputPtr < mInputEnd) ? mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_CDATA);
		int count = 0;
		...
		main_loop: while (true) {
				...
				if (c > CHAR_CR_LF_OR_NULL) {
					...
					if (c < CHAR_SPACE) {
						if (c == '\n') {
							...
							if (mInputPtr >= mInputEnd) {
								/*
								 * If we can't peek easily, let's flush past
								 * stuff and load more... (have to flush, since
								 * new read will overwrite inbut buffers)
								 */
								int len = mInputPtr - start;
								if (len > 0) {
									w.write(mInputBuffer, start, len);
							...
							if (d == '\n') {
								if (mNormalizeLFs) {
									/*
									 * Let's flush content prior to 2-char LF,
									 * and start the new segment on the second
									 * char... this way, no mods are needed for
									 * the buffer, AND it'll also work on split
									 * 2-char lf!
									 */
									int len = mInputPtr - 2 - start;
									if (len > 0) {
			...
			{
				/*
				 * -1 since the last char in there (a '[') is NOT to be output
				 * at this point
				 */
			...
			/*
			 * Ok; we only get this far when we hit a ']'. We got one, so let's
			 * see if we can find at least one more bracket, immediately
			 * followed by '>'...
			 */
			int bracketCount = 0;
			do {
			...
			/*
			 * Otherwise we'll just loop; now c is properly set to be the next
			 * char as well.
			 */
		} // while (true)
	...
	private int readAndWriteCoalesced(Writer w, boolean wasCData) throws IOException, XMLStreamException {
		...
		/*
		 * Ok, so what do we have next? CDATA, CHARACTERS, or something else?
		 */
		main_loop: while (true) {
			if (mInputPtr >= mInputEnd) {
				if (!loadMore()) {
					/*
					 * Shouldn't normally happen, but let's just let caller deal
					 * with it...
					 */
					break main_loop;
				}
			...
			if (c == '<') { // CDATA, maybe?
				...
				/*
				 * Did we hit an 'unexpandable' entity? If so, need to just bail
				 * out (only happens when Coalescing AND not expanding -- a
	...
	/*
	 * /////////////////////////////////////////////////////////// // Internal
	 * methods, low-level input access
	 * ///////////////////////////////////////////////////////////
	 */
	 ...
	 *
	 * @return true If at least one white space was skipped; false if not
	 *         (character passed was not white space)
	 */
	...
	protected EntityDecl findEntity(String id, Object arg) throws XMLStreamException {
		...
		/*
		 * 05-Mar-2006, TSa: Externally declared entities are illegal if we were
		 * declared as "standalone='yes'"...
		 */
	...
	protected void handleUndeclaredEntity(String id) throws XMLStreamException {
		throwParseError(((mDocStandalone == DOC_STANDALONE_YES) ? ErrorConsts.ERR_WF_GE_UNDECLARED_SA
				: ErrorConsts.ERR_WF_GE_UNDECLARED), id, null);
	}
	...
	protected void handleIncompleteEntityProblem(WstxInputSource closing) throws XMLStreamException {
		...
		throwParseError(
				"Unexpected end of entity expansion for entity &{0}; was expecting a close tag for element <{1}>",
				closing.getEntityId(), top);
	}

	/*
	 * /////////////////////////////////////////////////////////// // Internal
	 * methods, validation, error handling and reporting
	 * ///////////////////////////////////////////////////////////
	 */

	/**
	 * This problem gets reported if an entity tries to expand to a close tag
	 * matching start tag that did not came from the same entity (but from
	 * parent).
	 */
	protected void handleGreedyEntityProblem(WstxInputSource input) throws XMLStreamException {
		String top = mElementStack.isEmpty() ? "[ROOT]" : mElementStack.getTopElementDesc();
		throwParseError("Improper GE/element nesting: entity &" + input.getEntityId() + " contains closing tag for <"
				+ top + ">");
	}

	private void throwNotTextual(int type) {
		throw new IllegalStateException("Not a textual event (" + tokenTypeDesc(type) + ")");
	}

	private void throwNotTextXxx(int type) {
		throw new IllegalStateException("getTextXxx() methods can not be called on " + tokenTypeDesc(type));
	}

	protected void throwNotTextualOrElem(int type) {
		throw new IllegalStateException(
				MessageFormat.format(ErrorConsts.ERR_STATE_NOT_ELEM_OR_TEXT, new Object[] { tokenTypeDesc(type) }));
	}
	...
	/**
	 * Stub method implemented by validating parsers, to report content that's
	 * not valid for current element context. Defined at this level since some
	 * such problems need to be caught at low-level; however, details of error
	 * reports are not needed here.
	 * 
	 * @param evtType
	 *            Type of event that contained unexpected content
	 */
